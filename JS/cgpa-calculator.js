...
//When the regulation drop-down changes, this code updates the course drop-down with the courses available for the selected regulation.
regulationSelect.addEventListener('change', function () {
    const selectedRegulation = this.value; //refers to the element that triggered the event -- regulation select in this case
    courseSelect.innerHTML = '';  // Clear previous options
    for (const course in regulations[selectedRegulation]) {
        const option = document.createElement('option');
        //This line sets the value attribute of the newly created option element to the course name
        option.value = course;
...
function calculateCGPA() {
    ...
    for (let i = 1; i <= semesterCount; i++) {
        const semesterGPA = parseFloat(document.getElementById(`semester${i}`).value); //It dynamically constructs the ID of the input field using the current value of i
